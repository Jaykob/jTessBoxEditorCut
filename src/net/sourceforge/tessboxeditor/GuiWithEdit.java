...
public class GuiWithEdit extends GuiWithMRU {
    ...
    void jMenuItemInsertActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        if (selected.size() <= 0) {
         ...
         //   return;
            newBox = new TessBox("X", new Rectangle(30, 30, 60, 80), (short)0);
        } else if (selected.size() > 1) {
            ...
            index = this.boxes.toList().indexOf(box);
            index++;
            newBox = new TessBox(box.getChrs(), new Rectangle(box.getRect()), box.getPage());
            System.out.println("index " + index + ", page " + newBox.getPage());
        }
