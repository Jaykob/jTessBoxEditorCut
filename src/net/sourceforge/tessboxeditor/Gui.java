...
public class Gui extends javax.swing.JFrame {
    ...
    private String currentDirectory, outputDirectory, cutImagesOutputDirectory;
    ...
    private void initComponents() {
        ...
        jButtonCutImages.setText("Cut Images");
        jButtonCutImages.setToolTipText("Save individual character images");
        jButtonCutImages.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                jButtonSaveLogActionPerformed(evt);
            }
        });
        jPanelMain.add(jButtonCutImages);
        ...
        jButtonCutImages.setText("Cut Images"); // NOI18N
        jButtonCutImages.setToolTipText("Save individual character images"); // NOI18N
        jButtonCutImages.setFocusable(false);
        jButtonCutImages.setHorizontalTextPosition(javax.swing.SwingConstants.CENTER);
        jButtonCutImages.setVerticalTextPosition(javax.swing.SwingConstants.BOTTOM);
        jButtonCutImages.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                cutOutImages();
            }
        });
        jPanel4.add(jButtonCutImages);
...
//                jLabelStatus.setText(bundle.getString("Loading_completed"));
    ...
    private BufferedImage rotateImage(BufferedImage image, double angle) {
        ...
        transform.rotate(Math.toRadians(angle), image.getWidth()*0.5, image.getHeight()*0.5);
        Rectangle newRect = transform.createTransformedShape(new Rectangle(image.getWidth(), image.getHeight())).getBounds();

        double xDiff = 0.5 * ((double)newRect.width - image.getWidth());
        double yDiff = 0.5 * ((double)newRect.height - image.getHeight());
        ...
        BufferedImage cutImgRotR = new BufferedImage(newRect.width, newRect.height, image.getType());
        rotateOp.filter(image, cutImgRotR);

        return cutImgRotR;
    }

    private void cutOutImages() {
        if (saveCutFileDlg()) {
            boolean saveRot = saveRotatedDlg();

            for (int curPage = 0; curPage < imageList.size(); curPage++) {
                BufferedImage image = imageList.get(curPage);
                List<TessBox> curBoxes = boxPages.get(curPage).toList();

                for (TessBox box : curBoxes) {
                  //  System.out.println("Box: " + box.getChrs() + ", rect: " + box.getRect().toString());
                    // cut out image using according box
                    BufferedImage cutImg = image.getSubimage(box.getRect().x-1, box.getRect().y-1, box.getRect().width+2, box.getRect().height+2);

                    BufferedImage cutImgInv = new BufferedImage(cutImg.getWidth(), cutImg.getHeight(), BufferedImage.TYPE_BYTE_GRAY);
                    Graphics g = cutImgInv.getGraphics();
                    g.drawImage(cutImg, 0, 0, null);

                    // Comment out to invert images
                    /*
                    short[] invert = new short[256];
                    for (int i = 0; i < 256; i++)
                        invert[i] = (short)(255 - i);
                    final BufferedImageOp invertOp = new LookupOp(new ShortLookupTable(0, invert), null);
                    invertOp.filter(cutImgInv, cutImgInv);
                    */

                    // Create slightly rotated versions
                    BufferedImage cutImgRotR = rotateImage(cutImgInv, 10.0);
                    BufferedImage cutImgRotL = rotateImage(cutImgInv, -10.0);

                    // save image as tiff
                    int postfix = 0;
                    String boxChar = box.getChrs();
                    if (box.getChrs().startsWith("."))
                        boxChar = "point";
                    else if (box.getChrs().startsWith(","))
                        boxChar = "comma";
                    else if (box.getChrs().startsWith(":"))
                        boxChar = "colon";
                    else if (box.getChrs().startsWith("-"))
                    ...
                    String filename = cutImagesOutputDirectory + "/" + boxChar + "_" + postfix + ".tif";
                    File imgFile = new File(filename);
                    while (imgFile.exists()) {
                        postfix++;
                        filename = cutImagesOutputDirectory + "/" + boxChar + "_" + postfix + ".tif";
                        imgFile = new File(filename);
                    }
                 //   System.out.println("Filename: " + filename);

                    try {
                        ImageIO.write(cutImgInv, "tif", imgFile); // normal image

                        if (saveRot) { // rotated versions
                            String filenameR = filename.substring(0, filename.length() - 4) + "_r10.tif";
                            imgFile = new File(filenameR);
                            ImageIO.write(cutImgRotR, "tif", imgFile);

                            String filenameL = filename.substring(0, filename.length() - 4) + "_l10.tif";
                            imgFile = new File(filenameL);
                            ImageIO.write(cutImgRotL, "tif", imgFile);
                        }
                    }
                    catch (Exception e) {
                        e.printStackTrace();
    ...
    boolean saveCutFileDlg() {
        JFileChooser saveChooser = new JFileChooser(cutImagesOutputDirectory);
        FileFilter textFilter = new SimpleFilter("tif", "Tiff Files");
        saveChooser.setCurrentDirectory(new java.io.File("."));
        saveChooser.setFileSelectionMode(JFileChooser.DIRECTORIES_ONLY);
        //  saveChooser.addChoosableFileFilter(textFilter);
        //  saveChooser.setFileFilter(textFilter);
        saveChooser.setDialogTitle("Choose Directory to Save Images");
        saveChooser.setAcceptAllFileFilterUsed(false);

        if (saveChooser.showOpenDialog(this) == JFileChooser.APPROVE_OPTION) {
            cutImagesOutputDirectory = saveChooser.getSelectedFile().getAbsolutePath();
    ...
    boolean saveRotatedDlg() {
        int reply = JOptionPane.showConfirmDialog(null, "Save rotated chars (+/- 10°) besides regular chars?", "Save rotated?",
...
//            updateMRUList(boxFile.getPath());
    ...
    private void jButtonCutImagesActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonSaveActionPerformed
        saveCutFileDlg();
    }//GEN-LAST:event_jButtonSaveActionPerformed
    ...
    private javax.swing.JButton jButtonCutImages;
